...
function tambahJadwalPeriksa($data)
{
    try {
        ...
        $aktif = 'T'; // Default value is 'T'

        // Pengecekan apakah id_dokter sudah memiliki jadwal untuk hari yang sama
        $query_hari_sama = "SELECT * FROM jadwal_periksa 
                            WHERE id_dokter = '$id_dokter' AND hari = '$hari'";

        $result_hari_sama = mysqli_query($conn, $query_hari_sama);

        if (mysqli_num_rows($result_hari_sama) > 0) {
            return -2; // Kode indikasi jika jadwal untuk hari yang sama sudah ada
        }

        // Pengecekan apakah jadwal periksa untuk dokter ini bertumpang tindih pada hari yang sama
        $query_existing = "SELECT * FROM jadwal_periksa 
                            WHERE id_dokter = '$id_dokter' AND hari = '$hari' 
                            AND (
        ...
        if (mysqli_num_rows($checkJadwalPeriksa) > 0) {
            return -2; // Kode indikasi jika jadwal periksa sudah ada untuk dokter ini
        }

        // Pengecekan apakah ada jadwal aktif yang sudah ada untuk dokter ini
        $checkAktifQuery = "SELECT * FROM jadwal_periksa 
                            WHERE id_dokter = '$id_dokter' AND aktif = 'Y'";
        $result = mysqli_query($conn, $checkAktifQuery);

        if (mysqli_num_rows($result) > 0) {
            // Jika ada, perbarui status aktif mereka menjadi 'T' untuk dokter yang sama
            $updateAktifQuery = "UPDATE jadwal_periksa 
                                SET aktif = 'T' 
                                WHERE id_dokter = '$id_dokter' AND aktif = 'Y'";
            if (!mysqli_query($conn, $updateAktifQuery)) {
                echo "Error updating aktif: " . mysqli_error($conn);
                return -1; // Atau indikator kesalahan lainnya
            }
        }

        // Insert jadwal periksa baru
